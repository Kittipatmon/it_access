...
@section('title', 'จัดการลายมือชื่อพนักงาน')
@section('breadcrumb', 'จัดการลายมือชื่อพนักงาน')

@section('content')
    <div class="max-w-4xl mx-auto py-4" x-data="{ method: 'draw', loading: false, previewUrl: null, fileName: '' }">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">จัดการลายมือชื่อ ({{ $user->id === Auth::id() ? 'My Signature' : $user->fullname }})</h1>
                <p class="text-slate-500 mt-1 text-sm">{{ $user->id === Auth::id() ? 'บันทึกลายเซ็นของคุณไว้เพื่อใช้ในการส่งคำร้องและอนุมัติอย่างรวดเร็ว' : 'จัดการลายเซ็นสำหรับพนักงานในระบบ (Admin Mode)' }}</p>
            </div>
            <a href="{{ $user->id === Auth::id() ? route('request.index') : route('backend.users.index') }}"
                ...
                <span>กลับหน้า{{ $user->id === Auth::id() ? 'หลัก' : 'จัดการพนักงาน' }}</span>
            </a>
        </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 h-full">
                    ...
                    <div class="mt-6 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                        <h4 class="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-2">ข้อมูลพนักงาน</h4>
                        <p class="text-xs font-bold text-slate-700">{{ $user->fullname }}</p>
                        <p class="text-[10px] text-slate-500">รหัส: {{ $user->emp_code }}</p>
                    </div>
                </div>
            </div>
                                ...
                                <button type="submit" :disabled="loading"
                                    ...
                                    <template x-if="!loading">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span>บันทึกลายเซ็น (Admin บันทึกให้)</span>
                                        </div>
                                    </template>
                                    <template x-if="loading">
                                        <div class="flex items-center space-x-3">
